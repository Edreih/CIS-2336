<!-- Header -->
				<div id="header">
					<div>

						<!-- Logo -->
							<div id="logo">
								<h1><a href="/projects/cis2336/index.php">CIS 2336 Portfolio</a></h1>
							</div>

						<!-- Nav -->
							<nav id="nav">
								<ul>
									<li><a href="/projects/cis2336/index.php">Home</a></li>
									<li><a href="/projects/cis2336/module-1/index.php">Module 1</a></li>
									<li><a href="/projects/cis2336/module-2/index.php">Module 2</a></li>
									<li><a href="/projects/cis2336/module-3/index.php">Module 3</a></li>
									<li><a href="/projects/cis2336/module-4/index.php">Module 4</a></li>
                                    <li class="current_page_item"><a href="/projects/cis2336/module-5/index.php">Module 5</a></li>
								</ul>
							</nav>

					</div>
				</div>